...
export default class ResponseEnvelope{
	...
	static from(obj){
		...
		envelope.setPagination(ResponsePagination.from(obj._pagination));
	 ...
	 * @param {Number} paginated_result.total
	 * @returns {ResponseEnvelope}
	 */
	static paginated(paginated_result){
		// 'mongoose-paginate' specific
		return ResponseEnvelope.data(paginated_result.docs).setPagination(new ResponsePagination(paginated_result.total));
	}
